...
class _ActiveWorkoutScreenState extends ConsumerState<ActiveWorkoutScreen> {
  ...
  void _toggleSetDone(String exId, int i) {
    ...
    if (!wasChecked) {
      ...
      ref
          ...
          .startRestTimer(Duration(seconds: cfg.restSeconds));
    } else {
  ...
  void _startExerciseTimer(String exId, int setIndex) {
    ...
    _exerciseTimers[timerKey] = Timer.periodic(const Duration(seconds: 1), (_) {
      if (mounted) {
        setState(() {
          _exerciseTimerSeconds[timerKey] =
              (_exerciseTimerSeconds[timerKey] ?? 0) + 1;
  ...
  Future<void> _handleFinish() async {
    final save = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'Vuoi salvare i progressi o abbandonare senza salvare?',
        ),
        actions: [
          TextButton(
  ...
  Map<String, dynamic>? _getExerciseHistory(String exId, bool isDuration) {
    ...
    for (final log in logs) {
      final sets = log.sets
          .where((s) => s.exerciseId == exId && s.isCompleted)
          .toList();
      if (sets.isNotEmpty) {
        double maxWeight = 0;
        int maxMetric = 0;
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<WorkoutState>(workoutProvider, (prev, next) {
      if (next.restTimerJustFinished &&
          !(prev?.restTimerJustFinished ?? false)) {
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          leading: IconButton(
            ...
            onPressed: () =>
                ref.read(workoutProvider.notifier).minimizeWorkout(),
          ),
          title: GestureDetector(
            onTap: () async {
              final currentName =
                  workoutState.currentWorkout?.name ?? widget.routine.name;
              ...
              if (newName != null &&
                  newName.isNotEmpty &&
                  newName != currentName) {
                ref.read(workoutProvider.notifier).updateWorkoutName(newName);
              }
            },
            child: Container(
        ...
        body: ListView.builder(
          ...
          itemBuilder: (context, exIndex) {
            ...
            final hasNotes =
                (ex.notes.isNotEmpty) ||
                (cfg?.routineNotes.isNotEmpty ?? false);
            final isDuration = ex.metricType == MetricType.duration;
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                [
                                  if (cfg?.routineNotes.isNotEmpty ?? false)
                                    cfg!.routineNotes,
                    ...
                    Builder(
                      builder: (context) {
                        ...
                        final dateStr = DateFormat(
                          'dd MMM yyyy',
                          'it_IT',
                        ).format(history['date'] as DateTime);
                        final metricStr = isDuration
                            ? '${history['maxMetric']}s'
                            : '${history['maxMetric']} reps';
                        final weightStr = history['maxWeight'] > 0
                            ? ' @ ${history['maxWeight']}kg'
                            : '';
          ...
          final result = await showDialog<int>(
            ...
            builder: (ctx) => AlertDialog(
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: () =>
                      Navigator.pop(ctx, int.tryParse(ctrl.text) ?? seconds),
                  child: const Text('OK'),
                ),
              ],
            ),
